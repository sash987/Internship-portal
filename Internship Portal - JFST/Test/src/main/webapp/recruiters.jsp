...
try{
	...
	Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/test?useSSL=false", "root", "freedom!");
	Statement st = con.createStatement();
	ResultSet rs = st.executeQuery("select * from recruiters");
	if(rs.next()==false){
...
<!-- HIGHEST SALARY TABLE START -->
<h1>HIGHEST SALARY</h1>
<%
try{
	//establish connection with mysql database
	Class.forName("com.mysql.cj.jdbc.Driver");
	Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/companydb?useSSL=false", "root", "freedom!");
	Statement st = con.createStatement();
	String str="select companyname,stipend,ctc from companyinfo order by ctc desc limit 3";
...
%>
<!-- HIGHEST SALARY TABLE END  -->



<!-- HIGHEST STIPEND TABLE START -->
<h1>HIGHEST STIPEND</h1>
<%
try{
	//establish connection with mysql database
	Class.forName("com.mysql.cj.jdbc.Driver");
	Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/companydb?useSSL=false", "root", "freedom!");
	Statement st = con.createStatement();
	String str="select companyname,stipend,ctc from companyinfo order by stipend desc limit 3";
	ResultSet rs = st.executeQuery(str);
	if(rs.next()==false){
		out.println("<h1>No records found</h1>");
	}else{%>
